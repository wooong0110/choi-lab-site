...
사진 확대 화면의 좋아요와 댓글은 Cloudflare Worker 및 D1에 저장됩니다. 좋아요는 같은 사람이 여러 번 누를 수 있고, 댓글은 닉네임으로 작성합니다. **새 사진 추가 후 `gallery-api` 폴더에서 `npm run deploy`도 실행**해야 해당 사진에 좋아요·댓글을 저장할 수 있습니다. 설정·검증·댓글 관리 방법은 [gallery-api/README.md](gallery-api/README.md)를 참고하세요.
