...
기존 **대표 사진 + 날짜·제목 + 추가 사진 그리드**를 유지하며, 사진은 Google Drive에서 직접 불러옵니다. 사진을 누르면 Drive 원본이 새 탭에서 열립니다.

1. [gallery 폴더](https://drive.google.com/drive/folders/1wxAhTxApXKIkOmQFOrvnxgv3e2n5jiVO)에 사진을 업로드합니다. 행사별 하위 폴더로 정리해도 됩니다.
2. 해당 폴더와 사진을 **링크가 있는 모든 사용자 → 뷰어**로 공유합니다. 다른 드라이브 폴더를 공개할 필요는 없습니다.
3. 사진별 공유 링크를 복사해 `assets/js/gallery-data.js`의 해당 글 `images` 배열에 넣습니다. 첫 링크가 대표 사진입니다. AI에게 “gallery 폴더에 새로 올린 사진을 갤러리에 연결해줘”라고 요청해도 됩니다.
4. 제목·날짜·본문과 사진 링크를 확인하고 Commit → Push로 반영합니다. **폴더에 업로드만 해서는 새 사진이 자동 등록되지 않습니다.** GitHub에는 사진 대신 링크만 추가합니다.

한국어 제목·본문은 titleKo, bodyKo에 넣습니다. 이미지 로딩이 실패하면 원본 보기 링크가 표시됩니다.

기존 `assets/img/gallery/` 파일은 더 이상 갤러리에서 참조하지 않지만 원본 보존을 위해 남겨 두었습니다. 기존 파일을 삭제해도 과거 Git 커밋의 용량은 줄어들지 않습니다.
